      ...
      .post(stringData) (err, res, body) ->
        ...
        robot.http("https://#{lastline_analysis_domain}/analysis/submit/url")
          ...
          .post(stringData) (err, res, body) ->
            ...
            if (results.data.score == undefined) then msg.send "Are you sure that domain resolves? Also, we don't support file submissions via url; we're looking for browser exploits." else if final_result.data.score < 30 then msg.send "No worries, bro, #{url} doesn't seem to have any browser exploits." else msg.send "Man, that #{url} is badness. We found #{final_result.data.malicious_activity} in there."
