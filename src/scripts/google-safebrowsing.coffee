...
module.exports = (robot) ->
  robot.respond /gsafe (.*)/i, (msg) ->
    ...
    if GOOGLE_SAFEBROWSING_API_KEY?
      ...
      search_term = msg.match[1].toLowerCase()
      ...
      request_url += "url=#{encodeURIComponent search_term}"

      robot.http(request_url)
        .get() (err, res, body) ->

          if res.statusCode is 200
            msg.send ":-1: #{search_term} is listed as #{body} in Google Safebrowsing. There's more info at http://www.google.com/safebrowsing/diagnostic?site=#{encodeURIComponent search_term}."
          else
             msg.send ":+1:The URL #{search_term} is NOT listed as suspicious in Google Safebrowsing. If you really want to know more check out http://www.google.com/safebrowsing/diagnostic?site=#{encodeURIComponent search_term}"
